...
game:GetService('ReplicatedStorage').DefaultChatSystemChatEvents:WaitForChild('OnMessageDoneFiltering').OnClientEvent:Connect(function(msgdata)
	if debounce or not string.match(msgdata.Message,'>lyrics ') or string.gsub(msgdata.Message,'>lyrics','') == '' or game:GetService('Players')[msgdata.FromSpeaker] == game:GetService('Players').LocalPlayer then return end
...
task.spawn(function()
	while task.wait(60) do
		if not debounce then
			sendMessage('I am lyrics bot! Type ">lyrics SongName" and I will sing the song!')
		end
	end
end)
